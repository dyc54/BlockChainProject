...
function CheckDemandOrderHospital() {
    ...
    useEffect(() => {
        fetch("http://localhost:8080/hospital/getDemandOrder?hospitalId=" + localStorage.getItem("id"), {
            method: "GET",
            headers: {
        ...
        <div>
            <table className="table table-striped">
                ...
                <tbody>
                    {data && data.map((item) => {
                        return (
                            <tr key={item.Record.hospitalId}>
                                <td>{item.Record.supplierId}</td>
...
export default CheckDemandOrderHospital;